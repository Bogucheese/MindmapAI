...
- **Agent mode (experimental)**: hand the control flow to the model — it assembles the whole map itself with real tools (read notes / fetch pages / place nodes one by one / self-check fixes) until it declares the map done (hard tool-call and node caps as a safety net).
