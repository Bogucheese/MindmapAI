...
- **四种内容来源**：仅主题 / 粘贴文本 / 网页链接 / 文档文件（.md/.txt/.docx/.pdf
  本地提取，零依赖）。链接模式支持 GitHub 页面自动转 raw 直连、B 站视频页元
  数据解析，抓取失败自动走回退链（直连 → 内置本机中继 → 自定义/公共抓取代理）。
  经 `start.sh`/`start.cmd` 启动时本机中继开箱即用；其它托管方式可用零依赖的
  [本地抓取代理](ai/tools/local-fetch-proxy.mjs)。
- **评审管线**：提炼要点 → 架构导图 → 自动评审，四维度评分不达标时自动回炉重构
  一次；生成中可随时取消。
- **Agent 过程面板**：侧边栏实时展示 AI 的真实工作内容——提炼出的每条要点原文
  与引文、结构骨架、评审四维分数与评语、回炉原因、每张图表进度
  （AI → Agent 面板）。面板跟随编辑器的亮/暗主题与界面语言，切换即时生效。
...
- **总结与优化建议**：AI 菜单一键对当前导图/图表做单发评审——总结、要点、
  可执行的优化建议（引用实际节点名）流入面板，只读不改画布。
...
- 全部图表使用**确定性几何布局**，布局函数有单元测试保证零重叠；连线一律
  正交路由，流程图支持 decision 分支与回退（显式 edges 协议）。
- **统一规模上限**：每张图元素上限可调（0 = 按类型默认），作用于步骤/分支/
  事件等主集合，AI 预算同步。
- 类型名与说明随界面语言切换（中/英）。
...
- [x] 对话式侧边栏（Agent 过程面板 + 命令式修改；Agent 模式支持导图与思考图）
- [x] 浏览器版零配置链接抓取（内置本机抓取中继；公共/自建代理兜底）
- [x] 流程图分支连线（decision 分支/回退显式 edges + 全正交路由）
- [x] 导图总结与优化建议（AI → 总结与优化建议；对当前生成结果单发评审入面板）
- [x] 文档转导图（Markdown / Word / PDF 文件直读提取，走来源管线）
- [ ] Markdown 双向编辑
