...
export function buildDistillSystemPrompt(language: OutputLanguage, maxNotes: number): string {
  ...
  return [
    ...
    '- "content" is one condensed knowledge point (under 25 words); keep concrete facts, definitions, distinctions, and numbers; light normalization of wording is welcome.',
...
export function buildArchitectSystemPrompt(c: GenerationConstraints, skeletonText?: string): string {
  if (skeletonText == null || skeletonText === '') {
    ...
    return [
      ...
      '- The knowledge points are RAW MATERIAL, not a cage: reorganize, merge, generalize and add common-sense structure whenever it makes the map clearer — but never contradict the source facts.',
...
export function buildCritiqueSystemPrompt(): string {
  return [
    ...
    'Scoring 0-100:',
    '- grounding: do node labels CONTRADICT the source facts? Penalize distorted or fabricated factual claims. Reasonable generalizations, structural labels and common-sense completions beyond the notes are acceptable — a map is not a transcript.',
...
export function buildChartSystemPrompt(meta: ChartTypeMeta): string {
  ...
  return [
    ...
    arrowSemantics,
    '- The knowledge points are RAW MATERIAL: reorganize and generalize freely for a clearer chart; common-sense completion is welcome, but never contradict the source facts. When the content is thin, produce fewer but meaningful items within the minimum ranges.',
